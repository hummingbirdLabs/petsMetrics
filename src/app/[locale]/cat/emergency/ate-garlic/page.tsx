...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  return {
    title: t('ateGarlic.meta.title'),
    description: t('ateGarlic.meta.description'),
    keywords: t('ateGarlic.meta.keywords'),
    alternates: {
      canonical: `${SITE_URL}/${locale}/cat/emergency/ate-garlic/`,
    },
    openGraph: {
      title: t('ateGarlic.article.ogTitle'),
      description: t('ateGarlic.article.ogDescription'),
      url: `${SITE_URL}/${locale}/cat/emergency/ate-garlic/`,
      type: 'article',
      images: [{ url: `${SITE_URL}/og/home.webp`, width: 1200, height: 630, alt: t('ateGarlic.article.ogTitle') }],
    },
    twitter: {
...
export default async function CatAteGarlicPage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: 'catEmergency' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const articleHeadline = t('ateGarlic.article.headline');
  const articleDesc = t('ateGarlic.article.description');
  ...
  const faqs = t.raw('ateGarlic.faqs') as Array<{ question: string; answer: string }>;
  const faqSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
  ...
  const breadcrumbSchema = generateBreadcrumbJsonLd([
    ...
    { position: 3, name: t('ateGarlic.breadcrumbLabel'), item: '' },
  ]);

  const rows = t.raw('ateGarlic.riskAssessment.rows') as Array<{ form: string; amount: string; risk: string; action: string }>;
  const steps = t.raw('ateGarlic.steps') as Array<{ title: string; content?: string; items?: string[] }>;
  const relatedTools = t.raw('ateGarlic.relatedTools') as Array<{ name: string; url: string }>;
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <Breadcrumb
          items={[
            ...
            { label: t('ateGarlic.breadcrumbLabel') },
          ]}
        />
      </div>
      ...
      <article className="mx-auto max-w-4xl px-4 pb-16 sm:px-6 lg:px-8">
        <header className="mb-8">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-orange-100 px-4 py-2 text-sm font-semibold text-orange-800">
            ...
            {t('ateGarlic.banner.severityLabel')}
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            {t('ateGarlic.banner.title')}
          </h1>
          <p className="mt-3 text-lg text-gray-600">
            <span dangerouslySetInnerHTML={{ __html: t('ateGarlic.banner.subtitle') }} />
          </p>
        </header>
          ...
          <div className="mt-4 overflow-hidden rounded-lg border border-gray-200">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">{t('shared.labels.garlicForm')}</th>
