// i18n: catEmergency.ateChocolate
...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  return {
    title: t('ateChocolate.meta.title'),
    description: t('ateChocolate.meta.description'),
    keywords: t('ateChocolate.meta.keywords'),
    alternates: {
      canonical: `${SITE_URL}/${locale}/cat/emergency/ate-chocolate/`,
    },
    openGraph: {
      title: t('ateChocolate.article.ogTitle'),
      description: t('ateChocolate.article.ogDescription'),
      url: `${SITE_URL}/${locale}/cat/emergency/ate-chocolate/`,
      type: 'article',
      images: [{ url: `${SITE_URL}/og/home.webp`, width: 1200, height: 630, alt: t('ateChocolate.article.ogTitle') }],
    },
    twitter: {
...
export default async function CatAteChocolatePage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  ...
  const breadcrumbSchema = generateBreadcrumbJsonLd([
    ...
    { position: 3, name: t('ateChocolate.breadcrumbLabel'), item: '' },
  ]);

  const rows = t.raw('ateChocolate.riskAssessment.rows') as Array<{ form: string; amount: string; risk: string; action: string }>;
  const steps = t.raw('ateChocolate.steps') as Array<{ title: string; content?: string; items?: string[] }>;
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <Breadcrumb
          items={[
            ...
            { label: t('ateChocolate.breadcrumbLabel') },
          ]}
        />
      </div>
