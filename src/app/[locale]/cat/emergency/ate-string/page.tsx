...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  return {
    title: t('ateString.meta.title'),
    description: t('ateString.meta.description'),
    keywords: t('ateString.meta.keywords'),
    alternates: {
      canonical: `${SITE_URL}/${locale}/cat/emergency/ate-string/`,
    },
    openGraph: {
      title: t('ateString.article.ogTitle'),
      description: t('ateString.article.ogDescription'),
      url: `${SITE_URL}/${locale}/cat/emergency/ate-string/`,
      type: 'article',
      images: [{ url: `${SITE_URL}/og/home.webp`, width: 1200, height: 630, alt: t('ateString.article.ogTitle') }],
    },
    twitter: {
...
export default async function CatAteStringPage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: 'catEmergency' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const articleHeadline = t('ateString.article.headline');
  const articleDesc = t('ateString.article.description');
  ...
  const articleSchema = {
    ...
    citation: [
      { '@type': 'CreativeWork', name: 'AAFP Feline Gastrointestinal Guidelines', url: 'https://catvets.com/' },
    ],
  };

  const faqs = t.raw('ateString.faqs') as Array<{ question: string; answer: string }>;
  const faqSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
  ...
  const breadcrumbSchema = generateBreadcrumbJsonLd([
    ...
    { position: 3, name: t('ateString.breadcrumbLabel'), item: '' },
  ]);

  const steps = t.raw('ateString.steps') as Array<{ title: string; content?: string; items?: string[] }>;
  const relatedTools = t.raw('ateString.relatedTools') as Array<{ name: string; url: string }>;
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <Breadcrumb
          items={[
            ...
            { label: t('ateString.breadcrumbLabel') },
          ]}
        />
      </div>
      ...
      <article className="mx-auto max-w-4xl px-4 pb-16 sm:px-6 lg:px-8">
        <header className="mb-8">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-orange-100 px-4 py-2 text-sm font-semibold text-orange-800">
            ...
            {t('ateString.banner.severityLabel')}
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            {t('ateString.banner.title')}
          </h1>
          <p className="mt-3 text-lg text-gray-600">
            <span dangerouslySetInnerHTML={{ __html: t('ateString.banner.subtitle') }} />
          </p>
        </header>
