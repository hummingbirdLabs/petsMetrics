// i18n: catEmergency.ateFishingLine
...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  return {
    title: t('ateFishingLine.meta.title'),
    description: t('ateFishingLine.meta.description'),
    keywords: t('ateFishingLine.meta.keywords'),
    alternates: {
      canonical: `${SITE_URL}/${locale}/cat/emergency/ate-fishing-line/`,
    },
    openGraph: {
      title: t('ateFishingLine.article.ogTitle'),
      description: t('ateFishingLine.article.ogDescription'),
      url: `${SITE_URL}/${locale}/cat/emergency/ate-fishing-line/`,
      type: 'article',
      images: [{ url: `${SITE_URL}/og/home.webp`, width: 1200, height: 630, alt: t('ateFishingLine.article.ogTitle') }],
    },
    twitter: {
...
export default async function CatAteFishingLinePage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: 'catEmergency' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const articleHeadline = t('ateFishingLine.article.headline');
  const articleDesc = t('ateFishingLine.article.description');
  ...
  const faqs = t.raw('ateFishingLine.faqs') as Array<{ question: string; answer: string }>;
  const faqSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
  ...
  const breadcrumbSchema = generateBreadcrumbJsonLd([
    ...
    { position: 3, name: t('ateFishingLine.breadcrumbLabel'), item: '' },
  ]);

  const criticalWarningLines = t.raw('ateFishingLine.criticalWarning.lines') as unknown as string[];
  const rows = t.raw('ateFishingLine.riskAssessment.rows') as Array<{ form: string; amount: string; risk: string; action: string }>;
  const steps = t.raw('ateFishingLine.steps') as Array<{ title: string; content?: string; items?: string[] }>;
  const relatedTools = t.raw('ateFishingLine.relatedTools') as Array<{ name: string; url: string }>;
    ...
    <>
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <Breadcrumb
          items={[
            ...
            { label: t('ateFishingLine.breadcrumbLabel') },
          ]}
        />
      </div>
      ...
      <article className="mx-auto max-w-4xl px-4 pb-16 sm:px-6 lg:px-8">
        <header className="mb-8">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-red-200 px-4 py-2 text-sm font-semibold text-red-900">
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
            </svg>
            {t('ateFishingLine.banner.severityLabel')}
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            {t('ateFishingLine.banner.title')}
          </h1>
          <p className="mt-3 text-lg text-gray-600">
            <span dangerouslySetInnerHTML={{ __html: t('ateFishingLine.banner.subtitle') }} />
          </p>
        </header>

        <section aria-labelledby="emergency-heading" className="mb-8 rounded-lg border-2 border-red-300 bg-red-50 p-6">
          <h2 id="emergency-heading" className="text-xl font-bold text-red-900">{t('shared.titles.criticalWarning')}</h2>
          <div className="mt-3 space-y-2 text-gray-700">
            {criticalWarningLines.map((line, i) => (
          ...
          <div className="mt-4 overflow-hidden rounded-lg border border-gray-200">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">{t('shared.labels.exposureType')}</th>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">{t('shared.labels.symptomsPresent')}</th>
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {rows.map((row, i) => (
                  <tr key={i}>
                    ...
                    <td className="px-4 py-3 text-sm font-semibold text-red-600">{row.risk}</td>
          ...
          <ol className="mt-4 space-y-4">
            {steps.map((step, i) => (
              <li key={i} className="flex gap-4">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-red-700 text-sm font-bold text-white">{i + 1}</span>
                <div>
        ...
        <section aria-labelledby="science-heading" className="mb-8">
          ...
          <p className="mt-4 text-base text-gray-700">
            {t('ateFishingLine.science.content')}
          </p>
        </section>
