...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  const metaTitle = t('ateAlcohol.meta.title');
  const metaDesc = t('ateAlcohol.meta.description');
  const metaKw = t('ateAlcohol.meta.keywords');
  const ogTitle = t('ateAlcohol.article.ogTitle');
  const ogDesc = t('ateAlcohol.article.ogDescription');
  ...
  return {
    ...
    keywords: metaKw,
    alternates: {
      canonical: `${SITE_URL}/${locale}/dog/emergency/ate-alcohol/`,
    },
    openGraph: {
      title: ogTitle,
      description: ogDesc,
      url: `${SITE_URL}/${locale}/dog/emergency/ate-alcohol/`,
      type: 'article',
...
export default async function DogAteAlcoholPage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  ...
  const breadcrumbItem = t('ateAlcohol.breadcrumbLabel');
  const articleHeadline = t('ateAlcohol.article.headline');
  const articleDesc = t('ateAlcohol.article.description');
  ...
  const faqs = t.raw('ateAlcohol.faqs') as Array<{ question: string; answer: string }>;
  const faqSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
  ...
  const rows = t.raw('ateAlcohol.riskAssessment.rows') as Array<{ form: string; amount: string; risk: string; action: string }>;
  const steps = t.raw('ateAlcohol.steps') as Array<{ title: string; content?: string; items?: string[] }>;
  const timeline = t.raw('ateAlcohol.timeline') as Array<{ time: string; color: string; description: string }>;
  const relatedTools = t.raw('ateAlcohol.relatedTools') as Array<{ name: string; url: string }>;

  const vetDecisionEmergency = t.raw('ateAlcohol.vetDecision.emergency') as unknown as string[];
  const vetDecisionUrgent = t.raw('ateAlcohol.vetDecision.urgent') as unknown as string[];
  const vetDecisionMonitor = t.raw('ateAlcohol.vetDecision.monitor') as unknown as string[];
      ...
      <article className="mx-auto max-w-4xl px-4 pb-16 sm:px-6 lg:px-8">
        <header className="mb-8">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-red-100 px-4 py-2 text-sm font-semibold text-red-800">
            ...
            {t('ateAlcohol.banner.severityLabel')}
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            {t('ateAlcohol.banner.title')}
          </h1>
          <p className="mt-3 text-lg text-gray-600">
            <span dangerouslySetInnerHTML={{ __html: t('ateAlcohol.banner.subtitle') }} />
          </p>
        </header>
          ...
          <div className="mt-4 overflow-hidden rounded-lg border border-gray-200">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">{t('ateAlcohol.riskAssessment.formHeader')}</th>
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {rows.map((row, i) => (
                  <tr key={i}>
                    ...
                    <td className={`px-4 py-3 text-sm font-semibold ${row.risk === 'CRITICAL' || row.risk.toLowerCase() === 'high' ? 'text-red-600' : row.risk === 'Moderate' ? 'text-orange-600' : 'text-yellow-600'}`}>{row.risk}</td>
        ...
        <section aria-labelledby="toxicology-heading" className="mb-8">
          ...
          <div className="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-6">
            <p className="text-sm text-gray-700">
              <strong>{t('shared.labels.amountEaten')}:</strong> {t('ateAlcohol.toxicityData.minimum')}
              <br />
              <strong>LD50:</strong> {t('ateAlcohol.toxicityData.ld50')}
              <br />
              <strong>{t('shared.titles.theScience')}:</strong> {t('ateAlcohol.toxicityData.source')}
            </p>
            <p className="mt-3 text-sm text-gray-600">
              {t('ateAlcohol.toxicityData.example')}
            </p>
          </div>
        </section>
