...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  const metaTitle = t('ateAvocado.meta.title');
  const metaDesc = t('ateAvocado.meta.description');
  const metaKw = t('ateAvocado.meta.keywords');
  const ogTitle = t('ateAvocado.article.ogTitle');
  const ogDesc = t('ateAvocado.article.ogDescription');
  ...
  return {
    ...
    keywords: metaKw,
    alternates: {
      canonical: `${SITE_URL}/${locale}/dog/emergency/ate-avocado/`,
    },
    openGraph: {
      title: ogTitle,
      description: ogDesc,
      url: `${SITE_URL}/${locale}/dog/emergency/ate-avocado/`,
      type: 'article',
...
export default async function DogAteAvocadoPage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  ...
  const breadcrumbItem = t('ateAvocado.breadcrumbLabel');
  const articleHeadline = t('ateAvocado.article.headline');
  const articleDesc = t('ateAvocado.article.description');
  ...
  const faqs = t.raw('ateAvocado.faqs') as Array<{ question: string; answer: string }>;
  const faqSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
  ...
  const rows = t.raw('ateAvocado.riskAssessment.rows') as Array<{ form: string; amount: string; risk: string; action: string }>;
  const steps = t.raw('ateAvocado.steps') as Array<{ title: string; content?: string; items?: string[] }>;
  const timeline = t.raw('ateAvocado.timeline') as Array<{ time: string; color: string; description: string }>;
  const relatedTools = t.raw('ateAvocado.relatedTools') as Array<{ name: string; url: string }>;

  const vetDecisionEmergency = t.raw('ateAvocado.vetDecision.emergency') as unknown as string[];
  const vetDecisionUrgent = t.raw('ateAvocado.vetDecision.urgent') as unknown as string[];
  const vetDecisionMonitor = t.raw('ateAvocado.vetDecision.monitor') as unknown as string[];
      ...
      <article className="mx-auto max-w-4xl px-4 pb-16 sm:px-6 lg:px-8">
        <header className="mb-8">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-yellow-100 px-4 py-2 text-sm font-semibold text-yellow-800">
            ...
            {t('ateAvocado.banner.severityLabel')}
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            {t('ateAvocado.banner.title')}
          </h1>
          <p className="mt-3 text-lg text-gray-600">
            <span dangerouslySetInnerHTML={{ __html: t('ateAvocado.banner.subtitle') }} />
          </p>
        </header>
          ...
          <div className="mt-4 overflow-hidden rounded-lg border border-gray-200">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">{t('ateAvocado.riskAssessment.formHeader')}</th>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">{t('shared.labels.anySize')}</th>
        ...
        <section aria-labelledby="toxicology-heading" className="mb-8">
          ...
          <div className="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-6">
            <p className="text-sm text-gray-700">
              <strong>{t('shared.labels.amountEaten')}:</strong> {t('ateAvocado.toxicityData.minimum')}
              <br />
              <strong>LD50:</strong> {t('ateAvocado.toxicityData.ld50')}
              <br />
              <strong>{t('shared.titles.theScience')}:</strong> {t('ateAvocado.toxicityData.source')}
            </p>
            <p className="mt-3 text-sm text-gray-600">
              {t('ateAvocado.toxicityData.example')}
            </p>
          </div>
        </section>
          ...
          <div className="mt-4 space-y-3">
            {timeline.map((entry, i) => (
              <div key={i} className="flex gap-3">
                <span className={`shrink-0 rounded px-2 py-1 text-xs font-semibold ${entry.color === 'green' ? 'bg-green-100 text-green-800' : entry.color === 'yellow' ? 'bg-yellow-100 text-yellow-800' : 'bg-orange-100 text-orange-800'}`}>{entry.time}</span>
