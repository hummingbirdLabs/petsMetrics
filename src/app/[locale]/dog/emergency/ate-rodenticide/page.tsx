...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  return {
    title: t('ateRodenticide.meta.title'),
    description: t('ateRodenticide.meta.description'),
    keywords: t('ateRodenticide.meta.keywords'),
    alternates: {
      canonical: `${SITE_URL}/${locale}/dog/emergency/ate-rodenticide/`,
    },
    openGraph: {
      title: t('ateRodenticide.meta.title'),
      description: t('ateRodenticide.article.ogDescription'),
      url: `${SITE_URL}/${locale}/dog/emergency/ate-rodenticide/`,
      type: 'article',
      images: [{ url: `${SITE_URL}/og/home.webp`, width: 1200, height: 630, alt: t('ateRodenticide.meta.title') }],
    },
    twitter: {
...
export default async function DogAteRodenticidePage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  ...
  const articleSchema = {
    ...
    '@type': 'Article',
    headline: t('ateRodenticide.article.headline'),
    description: t('ateRodenticide.article.description'),
  ...
  const faqs = t.raw('ateRodenticide.faqs') as Array<{ question: string; answer: string }>;
  const faqSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
  ...
  const breadcrumbSchema = generateBreadcrumbJsonLd([
    ...
    { position: 3, name: t('ateRodenticide.breadcrumbLabel'), item: '' },
  ]);

  const riskHeaders = t.raw('ateRodenticide.riskAssessment.headers') as { type: string; activeIngredient: string; riskLevel: string; actionRequired: string };
  const riskRows = t.raw('ateRodenticide.riskAssessment.rows') as Array<{ type: string; activeIngredient: string; riskLevel: string; action: string }>;
  const steps = t.raw('ateRodenticide.steps') as Array<{ title: string; content: string }>;
  const relatedTools = t.raw('ateRodenticide.relatedTools') as Array<{ name: string; url: string }>;
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <Breadcrumb
          items={[
            ...
            { label: t('ateRodenticide.breadcrumbLabel') },
          ]}
        />
      </div>
      ...
      <article className="mx-auto max-w-4xl px-4 pb-16 sm:px-6 lg:px-8">
        <header className="mb-8">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-red-200 px-4 py-2 text-sm font-semibold text-red-900">
            ...
            {t('ateRodenticide.banner.severityLabel')}
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            {t('ateRodenticide.banner.title')}
          </h1>
          <p className="mt-3 text-lg text-gray-600">
            <span dangerouslySetInnerHTML={{ __html: t('ateRodenticide.banner.subtitle') }} />
          </p>
        </header>
          ...
          <div className="mt-4 overflow-hidden rounded-lg border border-gray-200">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">{riskHeaders.type}</th>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">{riskHeaders.activeIngredient}</th>
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {riskRows.map((row, idx) => (
                  <tr key={idx}>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700">{row.activeIngredient}</td>
                    <td className="px-4 py-3 text-sm font-semibold text-red-600">{row.riskLevel}</td>
          ...
          <ol className="mt-4 space-y-4">
            {steps.map((step, idx) => (
              <li key={idx} className="flex gap-4">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-red-700 text-sm font-bold text-white">{idx + 1}</span>
                <div>
        ...
        <section aria-labelledby="science-heading" className="mb-8">
          ...
          <p className="mt-4 text-base text-gray-700">
            {t('ateRodenticide.science.content')}
          </p>
        </section>
        ...
        <section aria-labelledby="related-heading" className="mb-8">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            {relatedTools.map((tool, idx) => (
              <a key={idx} href={tool.url} className="flex items-center gap-3 rounded-lg border border-gray-200 bg-white p-4 transition-all hover:shadow-md hover:border-red-500">
