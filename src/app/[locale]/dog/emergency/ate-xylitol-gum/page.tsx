...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  return {
    title: t('ateXylitolGum.meta.title'),
    description: t('ateXylitolGum.meta.description'),
    keywords: t('ateXylitolGum.meta.keywords'),
    alternates: {
      canonical: `${SITE_URL}/${locale}/dog/emergency/ate-xylitol-gum/`,
    },
    openGraph: {
      title: t('ateXylitolGum.meta.title'),
      description: t('ateXylitolGum.article.ogDescription'),
      url: `${SITE_URL}/${locale}/dog/emergency/ate-xylitol-gum/`,
      type: 'article',
      images: [{ url: `${SITE_URL}/og/home.webp`, width: 1200, height: 630, alt: t('ateXylitolGum.meta.title') }],
    },
    twitter: {
...
export default async function DogAteXylitolGumPage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  ...
  const articleSchema = {
    ...
    '@type': 'Article',
    headline: t('ateXylitolGum.article.headline'),
    description: t('ateXylitolGum.article.description'),
  ...
  const faqs = t.raw('ateXylitolGum.faqs') as Array<{ question: string; answer: string }>;
  const faqSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
  ...
  const breadcrumbSchema = generateBreadcrumbJsonLd([
    ...
    { position: 3, name: t('ateXylitolGum.breadcrumbLabel'), item: '' },
  ]);

  const riskHeaders = t.raw('ateXylitolGum.riskAssessment.headers') as { amountEaten: string; xylitolContent: string; riskLevel: string; actionRequired: string };
  const riskRows = t.raw('ateXylitolGum.riskAssessment.rows') as Array<{ amountEaten: string; xylitolContent: string; riskLevel: string; action: string }>;
  const steps = t.raw('ateXylitolGum.steps') as Array<{ title: string; content: string }>;
  const relatedTools = t.raw('ateXylitolGum.relatedTools') as Array<{ name: string; url: string }>;
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <Breadcrumb
          items={[
            ...
            { label: t('ateXylitolGum.breadcrumbLabel') },
          ]}
        />
      </div>
      ...
      <article className="mx-auto max-w-4xl px-4 pb-16 sm:px-6 lg:px-8">
        <header className="mb-8">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-red-200 px-4 py-2 text-sm font-semibold text-red-900">
            ...
            {t('ateXylitolGum.banner.severityLabel')}
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            {t('ateXylitolGum.banner.title')}
          </h1>
          <p className="mt-3 text-lg text-gray-600">
            <span dangerouslySetInnerHTML={{ __html: t('ateXylitolGum.banner.subtitle') }} />
          </p>
        </header>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">{riskHeaders.xylitolContent}</th>
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {riskRows.map((row, idx) => (
                  <tr key={idx}>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700">{row.xylitolContent}</td>
        ...
        <section aria-labelledby="science-heading" className="mb-8">
          ...
          <p className="mt-4 text-base text-gray-700">
            {t('ateXylitolGum.science.content')}
          </p>
        </section>
