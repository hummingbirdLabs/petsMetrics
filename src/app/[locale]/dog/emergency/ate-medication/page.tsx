...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  const metaTitle = t('ateMedication.meta.title');
  const metaDesc = t('ateMedication.meta.description');
  const metaKw = t('ateMedication.meta.keywords');
  const ogTitle = t('ateMedication.article.ogTitle');
  const ogDesc = t('ateMedication.article.ogDescription');
  ...
  return {
    ...
    keywords: metaKw,
    alternates: {
      canonical: `${SITE_URL}/${locale}/dog/emergency/ate-medication/`,
    },
    openGraph: {
      title: ogTitle,
      description: ogDesc,
      url: `${SITE_URL}/${locale}/dog/emergency/ate-medication/`,
      type: 'article',
...
export default async function DogAteMedicationPage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  ...
  const breadcrumbItem = t('ateMedication.breadcrumbLabel');
  const articleHeadline = t('ateMedication.article.headline');
  const articleDesc = t('ateMedication.article.description');
  ...
  const faqs = t.raw('ateMedication.faqs') as Array<{ question: string; answer: string }>;
  const faqSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
  ...
  const rows = t.raw('ateMedication.riskAssessment.rows') as Array<{ weight: string; amount: string; risk: string; action: string }>;
  const steps = t.raw('ateMedication.steps') as Array<{ title: string; content?: string }>;
  const timeline = t.raw('ateMedication.timeline') as Array<{ time: string; color: string; description: string }>;
  const relatedTools = t.raw('ateMedication.relatedTools') as Array<{ name: string; url: string }>;

  const vetDecisionEmergency = t.raw('ateMedication.vetDecision.emergency') as unknown as string[];
  const vetDecisionUrgent = t.raw('ateMedication.vetDecision.urgent') as unknown as string[];
  const vetDecisionMonitor = t.raw('ateMedication.vetDecision.monitor') as unknown as string[];
      ...
      <article className="mx-auto max-w-4xl px-4 pb-16 sm:px-6 lg:px-8">
        <header className="mb-8">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-red-100 px-4 py-2 text-sm font-semibold text-red-800">
            ...
            {t('ateMedication.banner.severityLabel')}
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            {t('ateMedication.banner.title')}
          </h1>
          <p className="mt-3 text-lg text-gray-600">
            <span dangerouslySetInnerHTML={{ __html: t('ateMedication.banner.subtitle') }} />
          </p>
        </header>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">Toxic Dose</th>
        ...
        <section aria-labelledby="science-heading" className="mb-8">
          ...
          <p className="mt-4 text-base text-gray-700">
            {t('ateMedication.science.content')}
          </p>
        </section>
        ...
        <section aria-labelledby="toxicology-heading" className="mb-8">
          ...
          <div className="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-6">
            <p className="text-sm text-gray-700">
              <strong>{t('shared.labels.amountEaten')}:</strong> {t('ateMedication.toxicityData.minimum')}
              <br />
              <strong>LD50:</strong> {t('ateMedication.toxicityData.ld50')}
              <br />
              <strong>{t('shared.titles.theScience')}:</strong> {t('ateMedication.toxicityData.source')}
            </p>
            <p className="mt-3 text-sm text-gray-600">
              {t('ateMedication.toxicityData.example')}
            </p>
          </div>
        </section>
