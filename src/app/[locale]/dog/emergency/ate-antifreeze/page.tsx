...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  const metaTitle = t('ateAntifreeze.meta.title');
  const metaDesc = t('ateAntifreeze.meta.description');
  const metaKw = t('ateAntifreeze.meta.keywords');
  const ogTitle = t('ateAntifreeze.article.ogTitle');
  const ogDesc = t('ateAntifreeze.article.ogDescription');
  ...
  return {
    ...
    keywords: metaKw,
    alternates: {
      canonical: `${SITE_URL}/${locale}/dog/emergency/ate-antifreeze/`,
    },
    openGraph: {
      title: ogTitle,
      description: ogDesc,
      url: `${SITE_URL}/${locale}/dog/emergency/ate-antifreeze/`,
      type: 'article',
...
export default async function DogAteAntifreezePage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  ...
  const breadcrumbItem = t('ateAntifreeze.breadcrumbLabel');
  const articleHeadline = t('ateAntifreeze.article.headline');
  const articleDesc = t('ateAntifreeze.article.description');
  ...
  const faqs = t.raw('ateAntifreeze.faqs') as Array<{ question: string; answer: string }>;
  const faqSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
  ...
  const steps = t.raw('ateAntifreeze.steps') as Array<{ title: string; content?: string }>;
  const relatedTools = t.raw('ateAntifreeze.relatedTools') as Array<{ name: string; url: string }>;
      ...
      <article className="mx-auto max-w-4xl px-4 pb-16 sm:px-6 lg:px-8">
        <header className="mb-8">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-red-100 px-4 py-2 text-sm font-semibold text-red-800">
            ...
            {t('ateAntifreeze.banner.severityLabel')}
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            {t('ateAntifreeze.banner.title')}
          </h1>
          <p className="mt-3 text-lg text-gray-600">
            <span dangerouslySetInnerHTML={{ __html: t('ateAntifreeze.banner.subtitle') }} />
          </p>
        </header>
        ...
        <section aria-labelledby="science-heading" className="mb-8">
          ...
          <p className="mt-4 text-base text-gray-700">
            {t('ateAntifreeze.science.content')}
          </p>
        </section>
