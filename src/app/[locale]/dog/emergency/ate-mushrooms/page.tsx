...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  const metaTitle = t('ateMushrooms.meta.title');
  const metaDesc = t('ateMushrooms.meta.description');
  const metaKw = t('ateMushrooms.meta.keywords');
  const ogTitle = t('ateMushrooms.article.ogTitle');
  const ogDesc = t('ateMushrooms.article.ogDescription');
  ...
  return {
    ...
    keywords: metaKw,
    alternates: {
      canonical: `${SITE_URL}/${locale}/dog/emergency/ate-mushrooms/`,
    },
    openGraph: {
      title: ogTitle,
      description: ogDesc,
      url: `${SITE_URL}/${locale}/dog/emergency/ate-mushrooms/`,
      type: 'article',
...
export default async function DogAteMushroomsPage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  ...
  const breadcrumbItem = t('ateMushrooms.breadcrumbLabel');
  const articleHeadline = t('ateMushrooms.article.headline');
  const articleDesc = t('ateMushrooms.article.description');
  ...
  const faqs = t.raw('ateMushrooms.faqs') as Array<{ question: string; answer: string }>;
  const faqSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
  ...
  const rows = t.raw('ateMushrooms.riskAssessment.rows') as Array<{ weight: string; amount: string; risk: string; action: string }>;
  const steps = t.raw('ateMushrooms.steps') as Array<{ title: string; content?: string }>;
  const timeline = t.raw('ateMushrooms.timeline') as Array<{ time: string; color: string; description: string }>;
  const relatedTools = t.raw('ateMushrooms.relatedTools') as Array<{ name: string; url: string }>;

  const vetDecisionEmergency = t.raw('ateMushrooms.vetDecision.emergency') as string[];
  const vetDecisionUrgent = t.raw('ateMushrooms.vetDecision.urgent') as string[];
  const vetDecisionMonitor = t.raw('ateMushrooms.vetDecision.monitor') as string[];
      ...
      <article className="mx-auto max-w-4xl px-4 pb-16 sm:px-6 lg:px-8">
        <header className="mb-8">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-red-100 px-4 py-2 text-sm font-semibold text-red-800">
            ...
            {t('ateMushrooms.banner.severityLabel')}
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            {t('ateMushrooms.banner.title')}
          </h1>
          <p className="mt-3 text-lg text-gray-600">
            <span dangerouslySetInnerHTML={{ __html: t('ateMushrooms.banner.subtitle') }} />
          </p>
        </header>
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {rows.map((row, i) => (
                  <tr key={i}>
                    ...
                    <td className={`px-4 py-3 text-sm font-semibold ${row.risk === 'CRITICAL' || row.risk === 'High' ? 'text-red-600' : row.risk === 'Moderate' ? 'text-orange-600' : 'text-green-600'}`}>{row.risk}</td>
        ...
        <section aria-labelledby="science-heading" className="mb-8">
          ...
          <p className="mt-4 text-base text-gray-700">
            {t('ateMushrooms.science.content')}
          </p>
        </section>
        ...
        <section aria-labelledby="toxicology-heading" className="mb-8">
          ...
          <div className="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-6">
            <p className="text-sm text-gray-700">
              <strong>{t('shared.labels.amountEaten')}:</strong> {t('ateMushrooms.toxicityData.minimum')}
              <br />
              <strong>LD50:</strong> {t('ateMushrooms.toxicityData.ld50')}
              <br />
              <strong>{t('shared.titles.theScience')}:</strong> {t('ateMushrooms.toxicityData.source')}
            </p>
            <p className="mt-3 text-sm text-gray-600">
              {t('ateMushrooms.toxicityData.example')}
            </p>
          </div>
        </section>
