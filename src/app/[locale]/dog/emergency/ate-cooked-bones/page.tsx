...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  const metaTitle = t('ateCookedBones.meta.title');
  const metaDesc = t('ateCookedBones.meta.description');
  const metaKw = t('ateCookedBones.meta.keywords');
  const ogTitle = t('ateCookedBones.article.ogTitle');
  const ogDesc = t('ateCookedBones.article.ogDescription');
  ...
  return {
    ...
    keywords: metaKw,
    alternates: {
      canonical: `${SITE_URL}/${locale}/dog/emergency/ate-cooked-bones/`,
    },
    openGraph: {
      title: ogTitle,
      description: ogDesc,
      url: `${SITE_URL}/${locale}/dog/emergency/ate-cooked-bones/`,
      type: 'article',
...
export default async function DogAteCookedBonesPage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  ...
  const breadcrumbItem = t('ateCookedBones.breadcrumbLabel');
  const articleHeadline = t('ateCookedBones.article.headline');
  const articleDesc = t('ateCookedBones.article.description');
  ...
  const articleSchema = {
    ...
    citation: [
      ...
      { '@type': 'CreativeWork', name: 'AAHA Guidelines', url: 'https://www.aaha.org/' },
    ],
  };

  const faqs = t.raw('ateCookedBones.faqs') as Array<{ question: string; answer: string }>;
  const faqSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
  ...
  const rows = t.raw('ateCookedBones.riskAssessment.rows') as Array<{ weight: string; amount: string; risk: string; action: string }>;
  const steps = t.raw('ateCookedBones.steps') as Array<{ title: string; content?: string }>;
  const timeline = t.raw('ateCookedBones.timeline') as Array<{ time: string; color: string; description: string }>;
  const relatedTools = t.raw('ateCookedBones.relatedTools') as Array<{ name: string; url: string }>;

  const vetDecisionEmergency = t.raw('ateCookedBones.vetDecision.emergency') as unknown as string[];
  const vetDecisionUrgent = t.raw('ateCookedBones.vetDecision.urgent') as unknown as string[];
  const vetDecisionMonitor = t.raw('ateCookedBones.vetDecision.monitor') as unknown as string[];
      ...
      <article className="mx-auto max-w-4xl px-4 pb-16 sm:px-6 lg:px-8">
        <header className="mb-8">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-orange-100 px-4 py-2 text-sm font-semibold text-orange-800">
            ...
            {t('ateCookedBones.banner.severityLabel')}
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            {t('ateCookedBones.banner.title')}
          </h1>
          <p className="mt-3 text-lg text-gray-600">
            <span dangerouslySetInnerHTML={{ __html: t('ateCookedBones.banner.subtitle') }} />
          </p>
        </header>
          ...
          <div className="mt-4 overflow-hidden rounded-lg border border-gray-200">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">{t('shared.labels.whatWasEaten')}</th>
        ...
        <section aria-labelledby="science-heading" className="mb-8">
          ...
          <p className="mt-4 text-base text-gray-700">
            {t('ateCookedBones.science.content')}
          </p>
        </section>
        ...
        <section aria-labelledby="toxicology-heading" className="mb-8">
          ...
          <div className="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-6">
            <p className="text-sm text-gray-700">
              <strong>{t('shared.labels.amountEaten')}:</strong> {t('ateCookedBones.toxicityData.minimum')}
              <br />
              <strong>LD50:</strong> {t('ateCookedBones.toxicityData.ld50')}
              <br />
              <strong>{t('shared.titles.theScience')}:</strong> {t('ateCookedBones.toxicityData.source')}
            </p>
            <p className="mt-3 text-sm text-gray-600">
              {t('ateCookedBones.toxicityData.example')}
            </p>
          </div>
        </section>
