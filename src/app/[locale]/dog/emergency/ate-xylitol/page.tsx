...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  const metaTitle = t('ateXylitol.meta.title');
  const metaDesc = t('ateXylitol.meta.description');
  const metaKw = t('ateXylitol.meta.keywords');
  const ogTitle = t('ateXylitol.article.ogTitle');
  const ogDesc = t('ateXylitol.article.ogDescription');
  ...
  return {
    ...
    keywords: metaKw,
    alternates: {
      canonical: `${SITE_URL}/${locale}/dog/emergency/ate-xylitol/`,
    },
    openGraph: {
      title: ogTitle,
      description: ogDesc,
      url: `${SITE_URL}/${locale}/dog/emergency/ate-xylitol/`,
      type: 'article',
...
export default async function DogAteXylitolPage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  ...
  const breadcrumbItem = t('ateXylitol.breadcrumbLabel');
  const articleHeadline = t('ateXylitol.article.headline');
  const articleDesc = t('ateXylitol.article.description');
  ...
  const faqs = t.raw('ateXylitol.faqs') as Array<{ question: string; answer: string }>;
  const faqSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
  ...
  const steps = t.raw('ateXylitol.steps') as Array<{ title: string; content?: string }>;
  const relatedTools = t.raw('ateXylitol.relatedTools') as Array<{ name: string; url: string }>;
      ...
      <article className="mx-auto max-w-4xl px-4 pb-16 sm:px-6 lg:px-8">
        <header className="mb-8">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-red-100 px-4 py-2 text-sm font-semibold text-red-800">
            ...
            {t('ateXylitol.banner.severityLabel')}
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            {t('ateXylitol.banner.title')}
          </h1>
          <p className="mt-3 text-lg text-gray-600">
            <span dangerouslySetInnerHTML={{ __html: t('ateXylitol.banner.subtitle') }} />
          </p>
        </header>
