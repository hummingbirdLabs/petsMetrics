...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  return {
    title: t('ateTobacco.meta.title'),
    description: t('ateTobacco.meta.description'),
    keywords: t('ateTobacco.meta.keywords'),
    alternates: {
      canonical: `${SITE_URL}/${locale}/dog/emergency/ate-tobacco/`,
    },
    openGraph: {
      title: t('ateTobacco.meta.title'),
      description: t('ateTobacco.article.ogDescription'),
      url: `${SITE_URL}/${locale}/dog/emergency/ate-tobacco/`,
      type: 'article',
      images: [{ url: `${SITE_URL}/og/home.webp`, width: 1200, height: 630, alt: t('ateTobacco.meta.title') }],
    },
    twitter: {
...
export default async function DogAteTobaccoPage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  ...
  const articleSchema = {
    ...
    '@type': 'Article',
    headline: t('ateTobacco.article.headline'),
    description: t('ateTobacco.article.description'),
  ...
  const faqs = t.raw('ateTobacco.faqs') as Array<{ question: string; answer: string }>;
  const faqSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
  ...
  const breadcrumbSchema = generateBreadcrumbJsonLd([
    ...
    { position: 2, name: t('shared.breadcrumb.dogEmergency'), item: `${SITE_URL}/dog/` },
    { position: 3, name: t('ateTobacco.breadcrumbLabel'), item: '' },
  ]);

  const riskHeaders = t.raw('ateTobacco.riskAssessment.headers') as { source: string; amount: string; riskLevel: string; actionRequired: string };
  const riskRows = t.raw('ateTobacco.riskAssessment.rows') as Array<{ source: string; amount: string; riskLevel: string; action: string }>;
  const steps = t.raw('ateTobacco.steps') as Array<{ title: string; content: string }>;
  const relatedTools = t.raw('ateTobacco.relatedTools') as Array<{ name: string; url: string }>;
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <Breadcrumb
          items={[
            ...
            { label: t('ateTobacco.breadcrumbLabel') },
          ]}
        />
      </div>
      ...
      <article className="mx-auto max-w-4xl px-4 pb-16 sm:px-6 lg:px-8">
        <header className="mb-8">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-red-100 px-4 py-2 text-sm font-semibold text-red-800">
            ...
            {t('ateTobacco.banner.severityLabel')}
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            {t('ateTobacco.banner.title')}
          </h1>
          <p className="mt-3 text-lg text-gray-600">
            <span dangerouslySetInnerHTML={{ __html: t('ateTobacco.banner.subtitle') }} />
          </p>
        </header>
          ...
          <div className="mt-4 overflow-hidden rounded-lg border border-gray-200">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">{riskHeaders.source}</th>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">{riskHeaders.amount}</th>
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {riskRows.map((row, idx) => (
                  <tr key={idx}>
                    ...
                    <td className={`px-4 py-3 text-sm font-semibold ${row.riskLevel === 'CRITICAL' ? 'text-red-600' : 'text-orange-600'}`}>{row.riskLevel}</td>
          ...
          <ol className="mt-4 space-y-4">
            {steps.map((step, idx) => (
              <li key={idx} className="flex gap-4">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-red-600 text-sm font-bold text-white">{idx + 1}</span>
                <div>
